...
class SiameseDataset(Dataset):
    def __init__(self, input_shape, dataset_path, num_train, num_val, train=True):
    ...
    def load_dataset(self):
        train_path = os.path.join(self.dataset_path, 'face_verification')
        # 遍历大循环(每个人)
        for character in os.listdir(train_path):
            # 对每个人每张图片进行遍历,存入一个list中
        ...
        random.seed(None)
        # 将train_lines与train_labels转换为np,并打乱顺序
    ...
    def _convert_path_list_to_images_and_labels(self, path_list):
        ...
        for pair in range(number_of_pairs):
            ...
            image = self.get_random_data(image, [self.image_height, self.image_width, self.channel])
            # image = self.get_random_data(image, self.input_shape)
            # 将Image转换成np.array [height, width, channel]
            image = np.asarray(image).astype(np.float64)
            # 将np.array转换成[channel, height, width]格式
            image = np.transpose(image, [2, 0, 1])
            # 归一化
            image = image / 255
            # 这段代码为了增强程序鲁棒性，其实因为之前get_random_data中已经把channels转换为3，这里根本用不上
            if self.channel == 1:
            ...
            #  设置标签
            # 显然：0,1(pair==0)为同一种类，2,3（(pair==1）为不同种类；4,5为同一种类，6,7为不同种类……
            if (pair + 1) % 2 == 0:
                labels[pair] = 0
            else:
                labels[pair] = 1
